# IE Brower Automation and testing library
# Copyright (C) 2004-03-03
#
...
import sys
#sys.path.append(r'c:\python3x\lib') - deprecated
...
class PAMIE:
    ...
    def changeWindow(self, wintext):
        ...
        newWin = self.findWindow(wintext)
    ...
    def clickMenu(self, tag, className, controlname, event=None):
        ...
        self._wait()
        try:
            doc = self._ie.Document.Body.getElementsByTagName(tag)
            for element in doc:
    ...
    def executeJavaScript(self, name):
            ...
            try:
                if self.frameName:
                  doc = self._ie.Document.frames[self.frameName].Document
                else:
                  doc = self._ie.Document
                pw = doc.parentWindow
                script = name
                print ("script"),script
                pw.execScript(script, 'JavaScript')
            except:
                (ErrorType,ErrorValue,ErrorTB)=sys.exc_info()
                print (sys.exc_info())
                traceback.print_exc(ErrorTB)
    ...
    def getElementsList(self, tag, filter=None, elementList=None):
        ...
        if elementList:
            ...
            if self.frameName:
                ...
                if self.formName:
                    elements = myFrame.Document.forms[self.formName].Body.getElementsByTagName(tag)
                else:
                    elements = myFrame.Document.Body.getElementsByTagName(tag)
            else:
                if self.formName:
                    elements = self._ie.Document.forms[self.formName].Body.getElementsByTagName(tag)
                else:
...
##            return None
    ...
    def getTableText(self,tableName,rownum,cellnum, frameName=None):
        ...
        """
        self._wait()
        table = self._ie.Document.Body.getElementsByTagName('table')
    ...
    def writeAttrs(self):
        ...
        for i in items:
            ...
            doc = self._ie.Document.Body.getElementsByTagName(i)
